...
function getChampsActifs(categorie: string, sousCategorie: string): string[] {
  ...
  if (categorie === 'immobilier') {
    ...
    if (sc === 'commerce')              return ['type_transaction', 'prix', 'surface', 'ville']
  ...
  const filtresContent = (
    <>
      ...
      <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        {/* Type de transaction — uniquement pour Commerce */}
        {champs.includes('type_transaction') && (
          <div>
        ...
        {/* Type de bien — options selon la sous-catégorie */}
        {champs.includes('type_bien') && (() => {
          const scLow = selectedSousCat.toLowerCase()
          const options = scLow === 'vente'
        ...
        {/* Chambres — masqué si seulement terrain sélectionné */}
        {champs.includes('nb_chambres') && valeurs.type_bien !== 'terrain' && (
          <div>
        ...
        {champs.includes('etat') && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {['neuf', 'bon état', 'à réparer'].map((val) => (
                <CheckboxItem key={val} label={val} active={isMultiActive('etat', val)} onClick={() => toggleMulti('etat', val)} />
              ))}
            </div>
          </div>
        )}
